...
def get_worker_cavity(server_name, worker_id):
    r = redis.StrictRedis(connection_pool=POOL)
    message = r.lindex("{}:cavity:{}".format(server_name, worker_id), 0)
    return deserialize_state(message)
...
ServerAPI = namedtuple("ServerAPI", ["get_worker_ids", "get_worker_state", "get_worker_cavity", "set_worker_cavity", "get_shared_state", "set_shared_state"])
...
def get_server_api(server_name):
    return ServerAPI(lambda: get_worker_ids(server_name),
                     ...
                     lambda worker_id: get_worker_cavity(server_name, worker_id),
